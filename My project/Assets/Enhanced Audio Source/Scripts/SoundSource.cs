...
public class SoundSource : MonoBehaviour
{
    ...
    public float offAxisFrequency = 3500f;
    ...
    public void Update(){
        ...
        if (listenerLocation){
            ...
            float distToPlayer = Vector3.Distance(listenerLocation.position, transform.position);
        ...
        if (angleToPlayer >= 0 && angleToPlayer <= 180){
            ...
            lowPassFilter.cutoffFrequency = evaluatedFreq;
        }

        evaluatedFreq = onAxisToOffAxisCurve.Evaluate(angleToPlayer);
        Keyframe[] keyframes = onAxisToOffAxisCurve.keys;
        keyframes[0].time = onAxisAngle / 2;
        keyframes[0].value = 20000;
        keyframes[1].time = offAxisAngle / 2;
        keyframes[1].value = offAxisFrequency;
    ...
    void TrackAngle(){
        Vector3 targetDir = listenerLocation.position - transform.position;
